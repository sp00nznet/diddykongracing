...
proc.on('exit', code => {
    // Parse output: split by "PC at" lines which delimit snapshots
    // Each snapshot has: PC line, then N mem responses as "(dbg) XXXXXXXX"
    ...
    for (const line of lines) {
        // PC line marks start of a new snapshot
        ...
        // Mem response: just 8 hex digits (possibly with (dbg) prefix)
        if (currentSnap) {
            // Match lines that are ONLY a hex value (with optional (dbg) prefix and whitespace)
    ...
    // Output formatted results - only show snapshots with the right number of values
    const numReads = STATE_READS.length;
    let snapIdx = 0;
    for (const snap of snapshots) {
        if (snap.values.length < numReads) continue; // skip incomplete snapshots
        snapIdx++;
        ...
        for (let r = 0; r < numReads && r < snap.values.length; r++) {
            ...
            let extra = '';
            // Check if this variable has labels defined
...
async function main() {
    await sleep(3000); // wait for init
    send('run');
    ...
    while ((Date.now() - startTime) < totalSec * 1000) {
        ...
        send('pause');
        await sleep(500);
        // Send a marker so we can align responses: read a known fixed address
        // Use PC command as a snapshot delimiter
        send('pc');
        await sleep(100);
        for (const [name, addr, len] of STATE_READS) {
            send(`mem ${addr} ${len}`);
            await sleep(50); // small delay between reads to avoid output interleaving
        }
        await sleep(300);
        send('run');
    }
